...
try:
    ...
    depts_resp   = requests.get(f"{API_URL}/departments", headers=get_headers())
    depts        = depts_resp.json() if depts_resp.status_code == 200 else []
    dept_options = {d["name"]: d["id"] for d in depts}

    # ─── CHOIX NOMBRE DE PROBLÈMES (User seulement) ──────
    if not is_admin():
        nb_problems = st.radio(
            "Nombre de problèmes à soumettre",
            options = [1, 2],
            index   = 0,
            horizontal = True
        )
    ...
    with st.form("rapport_form"):
        ...
        if nb_problems == 2:
            ...
            desc2 = st.text_area("Description", key="desc2",
                                  placeholder="Décrivez le deuxième problème...")
            col1, col2 = st.columns(2)
            with col1:
        ...
        if analyze_btn and desc1:
            ...
            if analyze_resp.status_code == 200:
                ...
                # ── Entités détectées (titre corrigé) ─────
                entities = result.get("entities", {})
                if any(entities.values()):
                    with st.expander("🔍 Entités détectées"):
